...
class Swap:
    ...
    def read_cv_im_and_landmarks(self, cv_img):
        im = cv_img
        im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
        im = cv2.resize(im, (im.shape[1] * self.SCALE_FACTOR,
                            im.shape[0] * self.SCALE_FACTOR))
...
class FaceSwap(Swap):
    ...
    def process(self, cv_img, emotion):
        """本体コードから呼ぶ関数"""
        s=Swap()
        ...
        im1, landmarks1 = s.read_im_and_landmarks(img)
        im2, landmarks2 = s.read_cv_im_and_landmarks(cv_img)

        M = s.transformation_from_points(landmarks1[s.ALIGN_POINTS],
                            landmarks2[s.ALIGN_POINTS])

        mask = s.get_face_mask(im2, landmarks2)
        warped_mask = s.warp_im(mask, M, im1.shape)
        combined_mask = numpy.max([s.get_face_mask(im1, landmarks1), warped_mask],
                        axis=0)

        warped_im2 = s.warp_im(im2, M, im1.shape)
        warped_corrected_im2 = s.correct_colours(im1, warped_im2, landmarks1)

        output_im = im1 * (1.0 - combined_mask) + warped_corrected_im2 * combined_mask
        
        ret = output_im.astype(numpy.uint8)
